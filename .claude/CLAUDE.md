...
| 報告功能 | 權限檢查複雜、資料格式多版本 | 權限不足錯誤多次出現 |
| 前端資料存取 | null/undefined 導致 crash | Internal Error 多次出現 |
...
**問題模式：** 用戶查看報告時出現「權限不足」或「無法載入報告資料」

**常見原因：**
1. 用戶的 `userProfile` 記錄不存在（新用戶）
2. 用戶的 `role` 為 null 或 `sales_rep`（非 manager/admin）
3. 用戶的 `department` 設定錯誤，無法查看特定部門報告
...
**相關檔案：**
- `packages/api/src/routers/analytics/rep-performance/index.ts` - 個人報告權限
- `packages/api/src/routers/analytics/team-performance/index.ts` - 團隊報告權限
- `packages/api/src/routers/analytics/mtd-uploads.ts` - MTD 上傳權限

**排查步驟：**
1. [ ] 確認用戶的 `userProfile` 是否存在
2. [ ] 確認用戶的 `role` 是否正確（manager/admin）
3. [ ] 確認用戶的 `department` 是否與目標一致
4. [ ] 檢查 API 回傳的錯誤訊息

---

### 網頁 Internal Error (2026-02-01)

**問題模式：** 前端頁面顯示「Internal Error」或空白

**常見原因：**
1. API 回傳 null/undefined，前端直接存取屬性
2. KV Cache 資料格式與程式碼預期不符
3. 新版/舊版資料格式不相容
