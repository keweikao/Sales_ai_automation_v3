	...
	handler: async (input: Input): Promise<Output> => {
		try {
			...
			if (input.includeMeddicBreakdown) {
				...
				meddicScores = {
					metrics: Number((meddicStats[0]?.avg_metrics as number)?.toFixed(1) || 0),
					economicBuyer: Number((meddicStats[0]?.avg_economic_buyer as number)?.toFixed(1) || 0),
					decisionCriteria: Number((meddicStats[0]?.avg_decision_criteria as number)?.toFixed(1) || 0),
					decisionProcess: Number((meddicStats[0]?.avg_decision_process as number)?.toFixed(1) || 0),
					identifyPain: Number((meddicStats[0]?.avg_identify_pain as number)?.toFixed(1) || 0),
					champion: Number((meddicStats[0]?.avg_champion as number)?.toFixed(1) || 0),
				};
			}
