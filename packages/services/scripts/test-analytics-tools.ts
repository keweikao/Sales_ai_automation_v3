...
async function testAnalyticsTools() {
	...
	try {
		...
		if (result.success) {
			...
			console.log(`  📄 File: ${(result.data as { filePath?: string }).filePath}`);
			console.log(`  📊 Rows: ${(result.data as { rowCount?: number }).rowCount}`);
		} else {
	...
	console.log(`  ℹ️  Database-dependent tests may be skipped in test environment`);
}
