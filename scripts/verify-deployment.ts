...
async function verifyDeployment() {
  ...
  try {
    ...
    if (
      metricPrompt.includes("客戶留存") ||
      metricPrompt.includes("預約填滿率")
    ) {
  ...
  try {
    ...
    // 檢查是否所有現有資料都是 ichef
    const nonIchefCount = dataCheck.rows.filter(
      (r) => r.product_line !== "ichef"
    ).length;
    if (nonIchefCount === 0) {
  ...
  if (allPassed) {
    ...
    console.log("下一步:");
    console.log(
      "  1. 部署 Queue Worker: cd apps/queue-worker && wrangler deploy"
    );
