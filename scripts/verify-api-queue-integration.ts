...
describe("API Router - Opportunity", () => {
  ...
  it("createOpportunitySchema 應該包含 productLine 欄位", async () => {
    ...
    const hasOptionalOrDefault =
      content.includes("productLine: z.enum") &&
      (content.match(/productLine:.*\.optional\(\)/s) !== null ||
        content.match(/productLine:.*\.default\(/s) !== null);
  ...
  it("listOpportunitiesSchema 應該支援 productLine 過濾", async () => {
    ...
    const hasProductLineFilter =
      content.includes("listOpportunitiesSchema") &&
...
describe("API Router - Conversation", () => {
  ...
  it("uploadConversationSchema 應該包含 productLine 欄位", async () => {
    ...
    // 檢查有 optional() 表示向後相容
    const hasOptional =
      content.includes("uploadConversationSchema") &&
  ...
  it("metadata 應該使用 passthrough() 支援額外欄位", async () => {
    ...
    const hasPassthrough =
      content.includes("metadata") && content.match(/\.passthrough\(\)/);
...
describe("Queue Worker", () => {
  ...
  it("Queue handler 應該解析 productLine 並提供預設值", async () => {
    ...
    const hasProductLineResolution =
  ...
  it("Queue handler 應該記錄 productLine 資訊", async () => {
    ...
    // 檢查有 log productLine
    const logsProductLine =
      content.includes("Product Line") || content.includes("productLine");
...
describe("向後相容性檢查", () => {
  it("所有 productLine 欄位都應該是 optional 或有 default", async () => {
    ...
    // 檢查 API schemas
    const oppContent = await fs.readFile(
      "./packages/api/src/routers/opportunity.ts",
      "utf-8"
    );
